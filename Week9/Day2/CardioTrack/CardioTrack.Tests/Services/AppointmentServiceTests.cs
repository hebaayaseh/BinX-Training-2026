...
namespace CardioTrack.Tests.Services
{
    public class AppointmentServiceTests
    {
        ...
        {
            // Arrange — Sqlite because AppointmentService opens a real transaction,
            ...
            connection.Open();
            var options = new DbContextOptionsBuilder<CardioTrackDbContext>().UseSqlite(connection).Options;
            await using var dbContext = new CardioTrackDbContext(options);
            dbContext.Database.EnsureCreated();

            var (doctor, nurse, patient) = await SeedBasicDataAsync(dbContext);
            var service = new AppointmentService(dbContext, new FakeAuditLog());

            var request = new AddDoctorScheduleRequestDto
            {
