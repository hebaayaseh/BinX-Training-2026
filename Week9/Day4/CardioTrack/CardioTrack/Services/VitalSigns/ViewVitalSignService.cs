...
namespace CardioTrack.Services.VitalSigns
{
    public class ViewVitalSignService : IVitalSign
    {
        ...
        public ViewVitalSignService(CardioTrackDbContext dbContext , VitalSignAlertEvaluator signAlertEvaluator , IAuditLog log)
        {
        ...
        public async Task<string> NurseResoleVitalSign(int userId, ResoleVitalSignAlertRequestDto request)
        {
            ...
            await log.LogAsync("Resolve vital signs ", "VitalSignAlerts", alert.Id, alert, alert);
            await dbContext.SaveChangesAsync();
            return "تم الحل بنجاح.";
        }
