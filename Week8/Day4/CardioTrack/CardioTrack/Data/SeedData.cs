...
namespace CardioTrack.Data
{
    public class SeedData
    {
        ...
        private async Task SeedBulkTestDataAsync()
        {
            if (await dbContext.patients.CountAsync() >= 50) return;

            var doctors = await dbContext.users.Where(u => u.Role == UserRole.Doctor).ToListAsync();
            var nurses = await dbContext.users.Where(u => u.Role == UserRole.Nurse).ToListAsync();
            ...
            for (int i = 0; i < 55; i++)
            {
                newPatients.Add(new Patient
                {
                    ...
                    DateOfBirth = DateTime.UtcNow.AddYears(-random.Next(20, 80)),
                    Gender = i % 2 == 0 ? Gender.Male : Gender.Female,
                    PhoneNumber = $"05900000{i:D2}",
            ...
            for (int i = 0; i < 150; i++)
            {
                ...
                vitalSigns.Add(new VitalSign
                {
                    ...
                    BloodPressureSystolic = random.Next(85, 160),
                    BloodPressureDiastolic = random.Next(55, 100),
                    OxygenSaturation = random.Next(85, 100),
                    Temperature = (decimal)(35.5 + random.NextDouble() * 3),
                    RecordedAt = DateTime.UtcNow.AddDays(-random.Next(1, 90)),
                    RecordedByUserId = nurses[random.Next(nurses.Count)].Id
                });
            }
            await dbContext.vitalSigns.AddRangeAsync(vitalSigns);
            await dbContext.SaveChangesAsync();
            ...
            for (int i = 0; i < 60; i++)
            {
                ...
                appointments.Add(new Appointment
                {
                    ...
                    AppointmentDate = DateTime.UtcNow.AddDays(random.Next(-30, 30)),
                    Reason = "Routine follow-up",
                    Status = AppointmentStatus.Scheduled,
                    CreatedByUserId = patient.DoctorId
                });
            }
            await dbContext.appointments.AddRangeAsync(appointments);
            await dbContext.SaveChangesAsync();
